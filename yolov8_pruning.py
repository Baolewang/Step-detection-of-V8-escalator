...
def train_v2(self: YOLO, pruning=False, **kwargs):
    ...
    if not overrides.get('data'):
        raise AttributeError("Dataset required but missing, i.e. pass 'data=D:/Project/pythonProject/ultralytics-8.0.89/ultralytics/datasets/pedal.yaml'")
...
def prune(args):
    ...
    pruning_cfg['data'] = "D:/Project/pythonProject/ultralytics-8.0.89/ultralytics/datasets/pedal.yaml"
    ...
    for i in range(args.iterative_steps):
        ...
        ######################### begin ###############################
        # === 新增：保存剪枝后（微调前）模型 ===
        pruned_dir = increment_path('pruned_models')
        os.makedirs(pruned_dir, exist_ok=True)

        # 在保存剪枝模型的地方替换为以下代码
        prune_save_path = os.path.join(pruned_dir, f'pruned_step_{i}_before_finetune.pt')

        # 创建完整的模型检查点结构
        ckpt = {
            'epoch': -1,
            'best_fitness': None,
            'model': deepcopy(pruner.model).half(),  # 半精度保存
            'ema': None,
            'updates': None,
            'optimizer': None,
            'train_args': getattr(model.model, 'args', {}),  # 获取原始训练参数
            'date': datetime.now().isoformat(),
            'version': __version__
        }

        # 保存完整检查点
        torch.save(ckpt, prune_save_path)

        # 添加元数据（YOLO所需）
        ckpt['model'].args = getattr(model.model, 'args', {})
        ckpt['model'].pt_path = prune_save_path
        ######################### end ###############################
    ...
    # 最后导出ONNX时添加路径
    onnx_save_path = os.path.join(pruned_dir, 'final_pruned_model.onnx')
    model.export(format='onnx',
                 simplify=True,
                 dynamic=False,
                 opset=13,
                 fp16=True,
                 file=onnx_save_path)  # 指定保存路径
    print(f"✅ Exported final ONNX model to {onnx_save_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', default='D:/Project/pythonProject/ultralytics-8.0.89/ultralytics/runs/detect/step_15_finetune/weights/last.pt', help='Pretrained pruning target model file')
    parser.add_argument('--cfg', default='default.yaml',
                        help='Pruning config file.'
                             ' This file should have same format with ultralytics/yolo/cfg/default.yaml')
    parser.add_argument('--iterative-steps', default=15, type=int, help='Total pruning iteration step')
